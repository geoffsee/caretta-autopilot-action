---
title: Post-mortem — CI-dispatch iteration / uncommitted working tree (diagnostic draft)
date: 2026-05-17
...
**Date:** 2026-05-17 | **Severity:** TODO | **Author:** TODO

**Status:** Resolved same day in commit `c7e0aa0`. The diagnostic draft was written *before* re-reading the 23-file diff. See **## Resolution (2026-05-17)** below, and the **Appendix** for the full text as it existed before this template pass.
...
- Two earlier post-mortems (`2026-05-15-autopilot-runci-gate-poll-stall-on-merge-conflict.md`, `2026-05-15-autopilot-example-repo-rulesets-workflow-permissions.md`) already flagged the underlying gap, and the fixes shipped since then have not closed it.
...
- TODO — Wall-clock times were not recorded for this write-up; approximate sequencing of commits appears under **## What we actually did (the 11 attempts)** below.
...
The diagnostic draft is preserved in **Appendix: Verbatim document (pre-template restructuring)**, because the *act of writing it* — and the contrast between its conclusions and the actual diff — is itself the most useful artifact in this file.
...
### What was broken (original diagnostic: concrete symptoms)
...
| Action | Owner | Due |
|--------|-------|-----|
| Snapshot the current state without landing it (`wip/ci-dispatch-2026-05-17` + local stash). | TODO | TODO |
| Write `.dev/docs/contracts/ci-dispatch.md` before further code changes. | TODO | TODO |
| Identify last commit that worked end-to-end in a representative consumer repo. | TODO | TODO |
| Decide rewrite vs. continue (module boundaries vs. single `CiDispatch` module). | TODO | TODO |
| Per-poll status table in wait loop (carried from 2026-05-15). | TODO | TODO |
| Move `dist/` build out of source commits / publish from CI. | TODO | TODO |
| Adopt "no fix without failing test against contract" for this surface. | TODO | TODO |
| Limit dispatch blast radius in `action-common` to primitives. | TODO | TODO |
...
## Appendix: Verbatim document (pre-template restructuring)
