...
**Date:** 2026-05-19 | **Severity:** TODO | **Author:** TODO
...
| Action | Owner | Due |
|--------|-------|-----|
| Surface DIRTY-but-filtered PRs in the run summary (`resolvedConflicts` bucket or similar). | TODO | TODO |
| Reconsider `filterAgentPRs`'s DIRTY exclusion — split "inventory" from "CI dispatch" concerns. | TODO | TODO |
| Add a behavioral contract test: DIRTY agent PR → `fix-conflicts` on this tick regardless of other PRs' CI state. | TODO | TODO |
| Verify against live state on `geoffsee/autopilot-example-project` (PR #142 across one or two ticks). | TODO | TODO |
...
3. **The cron is only a self-healer for things it can see.** The 2026-05-18 post-mortem (`2026-05-18-autopilot-pr-ci-snapshot-vs-rollup-blocked.md`) leaned heavily on "the next scan will reconcile." That property is conditional on the PR remaining visible to the scan. When the scan filters out the cases that need healing, repeated runs become repeated no-ops — which is exactly the user's symptom ("I have ran the autopilot several times and these branches do not get updated"). Any new filter should answer: "if a PR matches this filter, what *does* heal it?"
